...
##linear predictor
eta<-alpha+x*beta
##linear predictor logit E(Y) = eta
##Inverse function of the logit function is the logistic function,
##logistic(eta)=E(Y)=mu (here mu is a probability, often written pi)
pi<-exp(eta)/(1+exp(eta))
##Draw N from binomial distribution with parameters pi and 1
