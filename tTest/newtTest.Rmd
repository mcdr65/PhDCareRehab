---
title: "One-sample model and sample size"
...
# Continuous data: One-sample $t$-test
...
Of course, the results are the same. The $t$-test **is** a the simplest linear model.

Estimation of $\sigma$ "by hand" (this quantity is given in the output above):

```{r }
sqrt(sum(mod1$residuals^2)/(n-1)) 
```
...
n<-40 ##Sample Size
sigma<-20 ## True sigma
decision<-rep(0,nrep) ## Preparation of Decision vector
for (i in 1:nrep)
{
Y2 <- rnorm(n=n,mean=mu,sd=sigma) 
decision[i] <- (t.test(Y2,)$p.value)< alpha  ### nrep simulations and analysis
}
decision<-factor(decision,labels=c("do not reject H0","reject H0"))
decision <- prop.table(table(decision))
decision
```
...
```{r}
tlower<-(0.52-0.4)/(0.5/sqrt(300))
tlower
tupper<-(0.52-0.6)/(0.5/sqrt(300))
tupper
```
