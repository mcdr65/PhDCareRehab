...
knitr::opts_chunk$set(echo = T, out.width="50%",message=F, warning=F, comment=NA,
...
CasesKanton<-sapply(sKcases,function(x){x[length(x)]})
CasesKanton
sum(CasesKanton)
FatKanton<-sapply(sKfatal,function(x){x[length(x)]})
FatKanton
...
```{r fig1, fig.cap="Example of doubling times"}
swisspop<-8e6
time<-seq(1,60,by=1)
##################################################
cases<-c(100,114,214,268,337,374,491,652,652,1100,1359,
         2200,2200,2650,3000,4100,5300,6600,7500,8800,9900,10900,11800,12900,14100,14800,15900)
tag<-1:length(cases)
##################################################
T1<-1
T2<-2
...
plot(time,Y1,type="l",ylab="cases",ylim=c(100,20000),xlab="days",las=1)
...
```{r fig2, fig.cap="Example of doubling times, log scale"}
plot(time,Y1,log="y",type="l",ylab="cases",xlab="days",axes=FALSE)
...
lines(time,Y7,lty=3)
lines(time,Y3,col="red",lty=2)
abline(h=swisspop,col="red",lty=2)
points(tag,cases,type="l",col="red")
```



In Figure \@ref(fig:fig1) and Figure \@ref(fig:fig2), we see different doubling times: 1 day (solid), 2 days
(dashed), 3 days (red), seven days (dotted), with reported cases
Covid19 in Switzerland. Horizontal line: swiss population. **On a logarithmic scale, a straight line indicates exponential growth.**



#Auswirkung Vorfaktor

Annahme: 10 Prozent der Infizierten müssen ins Spital. Die Anzahl Cases von heute sind die
